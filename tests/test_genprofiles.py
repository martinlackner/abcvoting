...
def test_urn(num_cand, num_voters, setsize, replace):
    ...
    profile = genprofiles.random_urn_profile(
        num_cand, num_voters, setsize, replace)
...
@pytest.mark.parametrize(
    "num_parties,uniform",
    [(3, True), (3, False), (4, True), (5, False)]
)
def test_urn_party_list(num_cand, num_voters, num_parties,
                        replace, uniform):
    random.seed(0)
...
def test_IC(num_cand, num_voters, setsize):
    random.seed(0)
    profile = genprofiles.random_IC_profile(
        num_cand, num_voters, setsize)
    assert len(profile) == num_voters
    assert profile.num_cand == num_cand
    for pref in profile:
        assert len(pref) == setsize
...
def test_2d_party_list(num_cand, num_voters, num_parties, uniform, sigma):
    ...
    profile = genprofiles.random_2d_points_party_list_profile(
        num_cand, num_voters, num_parties, "uniform_square", "normal",
        sigma, uniform)
    assert len(profile) == num_voters
    assert profile.num_cand == num_cand
    assert profile.party_list()
