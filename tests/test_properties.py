...
@pytest.mark.parametrize("abc_yaml_instance", abc_yaml_filenames)
def test_maximin_support_nx_satisfies_properties_on_instances(abc_yaml_instance):
    """
    Exercise the new nx-max-flow implementation of Maximin-Support directly.

    For every YAML instance, we compute MMS winners with algorithm="nx-max-flow"
    and verify that each winning committee satisfies the key axiomatic properties
    we already assert for MMS elsewhere in this file (JR, PJR, priceability).

    Rationale: this ties the tests explicitly to the new flow-based backend so
    regressions in the max-flow construction (capacities, graph wiring, or tie
    handling) are caught even if the default "fastest" algorithm changes.
    """
    output.set_verbosity(verbosity=DETAILS)

    # read the instance from the file
    profile, committeesize, _, _ = fileio.read_abcvoting_yaml_file(abc_yaml_instance)

    # compute winners via nx-max-flow backend (unweighted)
    committees = abcrules.compute_maximin_support(
        profile,
        committeesize,
    ...
    # winning committees should satisfy these MMS-guaranteed properties (unweighted)
    for committee in committees:
        assert properties.check("jr", profile, committee)
        assert properties.check("pjr", profile, committee)
        assert properties.check("priceability", profile, committee)

    # Also test weighted case within the same test
    weighted_profile = profile.copy()
    original_length = len(weighted_profile)
    weighted_profile.convert_to_weighted()
    if len(weighted_profile) != original_length:
        committees_weighted = abcrules.compute_maximin_support(
            weighted_profile,
            committeesize,
            algorithm="nx-max-flow",
            resolute=False,
        )
        for committee in committees_weighted:
            assert properties.check("jr", weighted_profile, committee)
            assert properties.check("pjr", weighted_profile, committee)
            assert properties.check("priceability", weighted_profile, committee)


@pytest.mark.slow
@pytest.mark.parametrize("abc_yaml_instance", abc_yaml_filenames)
def test_mms_nx_vs_bruteforce_style_counterparts(abc_yaml_instance):
    """
    Brings the "brute-force vs X" style test to MMS by:
    - Comparing nx-max-flow vs gurobi counterparts for the MMS rule (no brute-force MMS available).
    - For a representative MMS winner, compare brute-force vs gurobi property checks
      (pareto, core, FJR, EJR, PJR), mirroring the structure from
      test_matching_output_different_approaches_and_implications.

    This ensures the nx-max-flow MMS backend is consistent with the gurobi backend,
    and that downstream axiomatic checks agree regardless of property solver.
    """
    output.set_verbosity(verbosity=DETAILS)

    profile, committeesize, compute_instances, _ = fileio.read_abcvoting_yaml_file(
        abc_yaml_instance
    )

    # MMS via nx-max-flow and gurobi should agree in irresolute mode
    nx_committees = abcrules.compute_maximin_support(
        profile, committeesize, algorithm="nx-max-flow", resolute=False
    )
    gurobi_committees = abcrules.compute_maximin_support(
        profile, committeesize, algorithm="gurobi", resolute=False
    )

    nx_set = {frozenset(c) for c in nx_committees}
    gurobi_set = {frozenset(c) for c in gurobi_committees}
    assert nx_set == gurobi_set

    # If YAML provides MMS results, also compare to those
    for ci in compute_instances:
        if ci.get("rule_id") == "maximin-support" and ci.get("result"):
            yaml_set = {frozenset(c) for c in ci["result"]}
            assert nx_set == yaml_set
            break

    # Cross-check properties on a representative MMS winner
    if nx_committees:
        committee = nx_committees[0]

        # The MMS winners not necessarily satisfy the properties, but they should be the same for the same input.
        # So we check that the properties return the same value False == False / True == True.

        # pareto
        assert properties.check_pareto_optimality(
            profile, committee, algorithm="brute-force"
        ) == properties.check_pareto_optimality(profile, committee, algorithm="gurobi")

        # core
        assert properties.check_core(
            profile, committee, algorithm="brute-force"
        ) == properties.check_core(profile, committee, algorithm="gurobi")

        # FJR
        assert properties.check_FJR(
            profile, committee, algorithm="brute-force"
        ) == properties.check_FJR(profile, committee, algorithm="gurobi")

        # EJR
        assert properties.check_EJR(
            profile, committee, algorithm="brute-force"
        ) == properties.check_EJR(profile, committee, algorithm="gurobi")

        # PJR
        assert properties.check_PJR(
            profile, committee, algorithm="brute-force"
        ) == properties.check_PJR(profile, committee, algorithm="gurobi")
